...
├─ app.py                       # エントリーポイント
├─ config/                      # 設定パッケージ + 同梱データ（import config は従来通り）
│   ├─ __init__.py              #   非機密の設定（バージョン・URL）
│   ├─ changelog.json           #   更新履歴（自動アップデートで配布）
│   └─ bundled_settings.json    #   配布時同梱の初期設定（PAT 等・gitignore）
├─ core/                        # 自動化ロジック
│   ├─ main.py                  #   全案件の実行制御
...
3. `config/bundled_settings_template.json` をコピーして `config/bundled_settings.json` を作成し、
...
```
# 1. コードを編集
# 2. config/__init__.py の APP_VERSION を上げる（例: 2.0.2）
...
- `credentials/`・`config/bundled_settings.json` は `.gitignore` 済み（リポジトリに入らない）。
